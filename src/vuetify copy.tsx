...
export class Vuetify implements UiInterface {
  constructor(target?: VuetifyProvider) {
    if (target) {
  ...
  icons: Icons = {
    add: "mdi-plus",
    columnsFilter: "mdi-tune",
    compact: "mdi-arrow-all",
    edit: "mdi-pencil",
    remove: "mdi-delete",
    search: "mdi-magnify",
    refresh: "mdi-refresh",
    ...
    refreshLeft: "mdi-rotate-left",
    refreshRight: "mdi-rotate-right",
    upload: "mdi-upload",
  ...
  formItem: FormItemCI = creator<FormItemCI>({
    ...
    injectFormItemContext() {
      const form = inject("vuetify:form", null) || {};
      return {
        async onChange() {
          await form.items?.value.find((o: { vm: FsUiFormItemContext }) => o.vm === this)?.validate();
        },
        async onBlur() {
          await form.items?.value.find((o: { vm: FsUiFormItemContext }) => o.vm === this)?.validate();
        }
